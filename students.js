...
//  3. Actualizar llave valor de todos los estudiantes agregando una nueva  //

db.students.updateMany({},{ $set: {"active": true,"modalidad": "Virtual"}})


//  4. Actualizar informacion de estudiantes (students)  //

db.students.updateOne({"code": 6503}, { $set: {"age": 20}});
db.students.updateOne({"code": 6714}, { $set: {"city": "Barranquilla"}});
db.students.updateOne({"code": 3875}, { $set: {"hobbies": "Volleyball"}});
db.students.updateOne({"code": 2354}, { $set: {"hobbies": ["Lectura", "Música"]}});
db.students.updateOne({"code": 7066}, { $set: {"hobbies": ["Fútbol", "Lectura"]}});
db.students.updateOne({"name": "Hilary Lee"}, { $set: {"address": "hilarylee09@pearlessa.com"}})


//  5. Cambiar nombre de llave: valor (modalidad - - modality)  //

db.students.updateMany({}, { $rename: { "modalidad": "modality" } })



//  6. Actualizar solo los estudiantes que tienen de ciudad bucarramanga  //

db.students.updateMany({ "ciudad": "Bucaramanga" },{ $set: { "modality": "on site" } })


//  7. Consultas de estudiantes y proyeccion  //

db.students.find({"city": "Barranquilla"}).count()
db.students.find({"edad": { $gt: 18 }}).count()
db.students.find({"city": { $ne: "Bogota" }}).count()
db.students.findOne({ "edad": 18 },{ "code": "", "name": "", "hobbies": "" })
db.students.findOne({ "edad": { $lt: 18 } },{ "code": "", "name": "", "edad": "" })


//  8. Borrar propiedad active de todos los documentos  //

db.students.updateMany( {} , { $unset: { "activo": "" } } )


//  9. Eliminar Hobbie de ciclismo de un estudiante (code : 2354)  //

db.students.updateOne( { "code": 2354 } , { $pull: { "hobbies": "Ciclismo" } } )


// 10. Eliminar hobbies de lectura y senderismo de un estudiante (code : 3875)  //

db.students.updateOne( { "code": 3875 } , { $pull: { "hobbies": { $in: ["Lectura", "Senderismo"] } } } )


// 11. Eliminar un estudiante (code : 9241)  //

db.students.deleteOne( { "code": 9241 } )



//  12. Eliminar los estudiante de Barranquilla  //

db.students.deleteMany({ "ciudad": "Barranquilla" })
